...
# Функция для объединения данных компании и рыночных индексов
...
# Функция дообучения и предсказания
def fine_tune_and_predict(model, live_data_file, market_live_file, scaler, sequence_length, columns_to_use, target_index):
    ...
    extended_sequence_length = max(sequence_length, 120)
    X_new, y_new = create_sequences(scaled_live_data, extended_sequence_length, target_index)
    if len(X_new) > 0:
        model.fit(X_new, y_new, epochs=5, batch_size=1, shuffle=False)
...
def main():
    ...
    while True:
        try:
            ...
            logger.error(f"Ошибка в процессе предсказания и дообучения: {e}")
